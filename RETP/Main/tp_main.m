...
while 1
    ...
    while 1
        ...
        seff_dt(seff_dt > 1) = 1;
        krw_dt = seff_dt .^ 0.5 .* (1 - (1 - (seff_dt .^ (1 ./ m))) .^ m) .^ 2;
        krn_dt = (1 - seff_dt) .^ 0.5 .* ((1 - seff_dt .^ (1 ./ m)) .^ m) .^ 2;
    ...
    pc_mas = pcm .* (seff_mas .^ (-1 ./ m) -1) .^ (1 ./ n);
    krw_mas = seff_mas .^ 0.5 .* (1 - (1 - (seff_mas .^ (1 ./ m))) .^ m) .^ 2;
    krn_mas = (1 - seff_mas) .^ 0.5 .* ((1 - seff_mas .^ (1 ./ m)) .^ m) .^ 2;
    ...
    if T >= T_rec(recond_index)
        ...
        pc_pc = pcm .* (seff_pc .^ (-1 ./ m) -1) .^ (1 ./ n);
        pc_pcn = pcm .* (seff_pcn .^ (-1 ./ m) -1) .^ (1 ./ n);
