...
% Value
n(:,:) = sampled_params.n(ii);
m = 1 - 1 ./ n;
a(:,:) = sampled_params.alpha(ii);
pcm = rhow0 * g ./ a;
k(:,:) = sampled_params.k(ii) * 0.01 / 86400 / (rhow0 * g / vw);
phi(:,:) = 0.43;
thewr(:,:) = 0;
thenr(:,:) = 0;
thews(:,:) = phi - thenr;
...
if sampled_params.k(ii) >= 200 && sampled_params.k(ii) <= 400
    dt_max = 10;
elseif sampled_params.k(ii) >= 100 && sampled_params.k(ii) < 200
    dt_max = 20;
elseif sampled_params.k(ii) >= 50 && sampled_params.k(ii) < 100
    dt_max = 40;
elseif sampled_params.k(ii) >= 20 && sampled_params.k(ii) < 50
    dt_max = 80;
elseif sampled_params.k(ii) >= 10 && sampled_params.k(ii) < 20
    dt_max = 160;
elseif sampled_params.k(ii) >= 5 && sampled_params.k(ii) < 10
    dt_max = 320;
elseif sampled_params.k(ii) < 5
    dt_max = 640;
else
    error('k 值超出范围'); 
end
...
inu_limit = 10;
% Recond time points 
T_ran = [0 logspace(log10(0.001),log10(1),20)]';
T_ran = [T_ran(1:19); 0.5; T_ran(20:end)];
