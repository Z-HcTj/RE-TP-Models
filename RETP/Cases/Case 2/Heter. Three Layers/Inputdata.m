...
% Value
n(:,:) = key_param(1,3*ii-2);
n(32:61,:) = key_param(1,3*ii-1);
n(62:end,:) = key_param(1,3*ii);
m = 1 - 1 ./ n;
a(:,:) = key_param(2,3*ii-2);
a(32:61,:) = key_param(2,3*ii-1);
a(62:end,:) = key_param(2,3*ii);
pcm = rhow0 * g ./ a;
k(:,:) = key_param(2,3*ii-2) * 0.01 / 86400 / (rhow0 * g / vw);
k(32:61,:) = key_param(2,3*ii-1) * 0.01 / 86400 / (rhow0 * g / vw);
k(62:end,:) = key_param(2,3*ii) * 0.01 / 86400 / (rhow0 * g / vw);
phi(:,:) = 0.43;
thewr(:,:) = 0;
thenr(:,:) = 0;
thews(:,:) = phi - thenr;
