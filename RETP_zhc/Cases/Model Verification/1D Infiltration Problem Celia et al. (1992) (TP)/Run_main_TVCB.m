...
while 1
    ...
    while 1
        ...
        % Permeability (intercell)
        Kw_mas = Kwtvcb(thew,Aw,Bw,g);
        Kn_mas = Kntvcb(Kas,Aa,Ba,pc_mas/(rhow0*g),g,rhon0,rhow0);
        ...
        dqw = dqw + sum(qw(1,SSq_left+1:SSq_right+1) * dt) - ...
            sum(qw(end,2:end-1) * dt);
        dmqw = (thew(2:end-1,2:end-1)) .* zgs .* rhow_mas(2:end-1,2:end-1) ...
            - (thew(2:end-1,1)) .* zgs .* rhow_mas(2:end-1,2:end-1);
