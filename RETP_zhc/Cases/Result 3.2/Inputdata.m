...
nz = length(z);
% Grid size
xgs = 0.2 * ones(length(z),length(x));
zgs = 0.2 * ones(length(z),length(x));
% Distance between points
dxp = 0.2 * ones(length(z) + 2,length(x) + 1);
dzp = 0.2 * ones(length(z) + 1,length(x) + 2);
%% Perturbation
