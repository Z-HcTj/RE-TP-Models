clear
clc

%% Data
Output_name = {'1_FluxBC_Unsat_results.mat', '2_FluxBC_Ponding_results.mat'};
% RE water content
re_thew_plot = [];
% TP water content
tp_thew_plot = [];
% RID
RID_index_plot = [];
% Mass balance 
re_wmass = [];
tp_wmass = [];

for ii = 1 : length(Output_name)
    load(Output_name{ii});
    re_thew_plot = [re_thew_plot ds.re_thew(:,2,:)];
    tp_thew_plot = [tp_thew_plot ds.tp_thew(:,2,:)];
    re_sum = [];
    tp_sum = [];
    for jj = 1 : length(ds.trec)
    re_seff = (re_thew_plot(2:end-1,ii,jj) - ds.thewr(2:end-1,2)) ...
        ./ (ds.phi(2:end-1,2) - ds.thewr(2:end-1,2));
    tp_seff = (tp_thew_plot(2:end-1,ii,jj) - ds.thewr(2:end-1,2)) ...
        ./ (ds.phi(2:end-1,2) - ds.thewr(2:end-1,2));
    re_sum = [re_sum; sum(re_seff)];
    tp_sum = [tp_sum; sum(tp_seff)];
    end
    RID_index = (re_sum - tp_sum) ./ (re_sum(end) - re_sum(1));
    RID_index_plot = [RID_index_plot RID_index];
    re_wmass = [re_wmass ds.re_wmass];
    tp_wmass = [tp_wmass ds.tp_wmass];
end
RID_index_plot = RID_index_plot * 100;

%% Water content profiles
z = [0.005:-0.01:-1.005]';
colors = [1 0.5 0; 0.53 0.81 0.98];
re_wc_name = {"RE 0.5T_{tot}" "RE 1.0T_{tot}"};
tp_wc_name = {"TP 0.5T_{tot}" "TP 1.0T_{tot}"};
figure
hold on
for ii = 1
    plot(z,re_thew_plot(:,ii,20),'Color',colors(1,:),'LineStyle','-','DisplayName',re_wc_name{1})
    plot(z,re_thew_plot(:,ii,end),'Color',colors(2,:),'LineStyle','-','DisplayName',re_wc_name{2})
    plot(z,tp_thew_plot(:,ii,20),'Color',colors(1,:),'LineStyle','--','DisplayName',tp_wc_name{1})
    plot(z,tp_thew_plot(:,ii,end),'Color',colors(2,:),'LineStyle','--','DisplayName',tp_wc_name{2})
end
hold off
view(90,-90)
ylim([0 0.45])
xlim([-1 0])
legend
title('LR Water Content Profiles')
xlabel('Z [m]')
ylabel('Water Content')

figure
hold on
for ii = 2
    plot(z,re_thew_plot(:,ii,20),'Color',colors(1,:),'LineStyle','-','DisplayName',re_wc_name{1})
    plot(z,re_thew_plot(:,ii,end),'Color',colors(2,:),'LineStyle','-','DisplayName',re_wc_name{2})
    plot(z,tp_thew_plot(:,ii,20),'Color',colors(1,:),'LineStyle','--','DisplayName',tp_wc_name{1})
    plot(z,tp_thew_plot(:,ii,end),'Color',colors(2,:),'LineStyle','--','DisplayName',tp_wc_name{2})
end
hold off
view(90,-90)
ylim([0 0.45])
xlim([-1 0])
legend
title('HR Water Content Profiles')
xlabel('Z [m]')
ylabel('Water Content')

%% RID plot
RID_name = {"FluxBC_Unsat" "FluxBC_Ponding"};
figure
hold on
for ii = 1: length(RID_name)
    plot(ds.trec,RID_index_plot(:,ii),'Color',colors(ii,:),'LineStyle','-','DisplayName',RID_name{ii})
end
hold off
legend
xlabel('Normalized Time')
ylabel('RID [%]')

